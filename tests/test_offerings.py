...
def test_title_is_required(ctx, provider):
    from offerings import create, OfferingDraft as Draft, OfferingError
    with pytest.raises(OfferingError):
        create(provider["id"], Draft(title="   "))


def test_duration_must_tile_a_grid(ctx, provider):
    from offerings import create, OfferingDraft as Draft, OfferingError, SLOT_MULTIPLES
    with pytest.raises(OfferingError):
...
def test_zero_price_is_allowed(ctx, provider):
    from offerings import create, OfferingDraft as Draft, get
...
def test_list_hides_deactivated_by_default(ctx, provider):
    from offerings import create, OfferingDraft as Draft, deactivate, list_for_provider
...
def test_deactivate_never_deletes(ctx, provider):
    ...
    from offerings import create, OfferingDraft as Draft, deactivate, get
...
def test_cannot_touch_someone_elses_offering(ctx, provider, client):
    from offerings import create, OfferingDraft as Draft, update, deactivate, OfferingError
...
def test_update_validates_the_merged_row(ctx, provider):
    ...
    from offerings import create, OfferingDraft as Draft, update, OfferingError
...
def test_update_with_nothing_to_change_is_a_no_op(ctx, provider):
    from offerings import create, OfferingDraft as Draft, update
...
def test_grouping_follows_the_category_order(ctx, provider):
    from offerings import create, OfferingDraft as Draft, grouped_for_provider, CATEGORIES
...
def test_unknown_category_still_appears(ctx, provider):
    ...
    from offerings import create, OfferingDraft as Draft, grouped_for_provider
...
def test_public_view_formats_money_and_flags_free(ctx, provider):
    from offerings import create, OfferingDraft as Draft, get, public_view
